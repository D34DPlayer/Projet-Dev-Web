...
const products = [
    {
        ...
        "photos": ["https://www.boucherie-vangeebergen.be/media/images/gallery/10/big/11.jpg"],
        "price": 50.0,
        "promo_price": 25.0,
        "price_type": "/kilo",
        "visibility": true
    },
    {
        ...
        "photos": ["https://www.boucherie-vangeebergen.be/media/images/gallery/10/big/12.jpg"],
        "price": 45.0,
        "promo_price": null,
        "price_type": "/unité",
        "visibility": false
...
describe("ProductsAdmin.vue", () => {
    ...
    it("renders the name correctly", () => {
        expect(firstRow.find('td[aria-colindex="1"]').text()).toMatch(products[0].name);
    });

    it("renders the category correctly", () => {
        expect(firstRow.find('td[aria-colindex="2"]').text()).toMatch(products[0].categorie);
    });
    ...
    it("shows the description on detail", () => {
        expect(wrapper.find('.b-table-details').text()).toMatch(products[0].description);
    });

    it("shows the photo(s) on detail", () => {
        expect(wrapper.findAll('.b-table-details .images-item').length).toBe(1);
        expect(wrapper.findAll('.b-table-details .images-item img').wrappers[0].attributes().src)
