<template>
  ...
  <b-form @submit="onSubmit">
    ...
    >
      <b-input-group>
        <b-form-input
            ...
            type="number"
            placeholder="Entrez la promo du produit"
        ></b-form-input>
        <b-form-select
            v-model="form.price_type"
            disabled
    ...
    <!-- Input Affichage -->
    <b-form-group
        label="Afficher en tant que produit de la semaine? :"
    ...
    <!-- Input Images -->
    <!-- Bouton submit -->
    <b-button type="submit">{{ edit ? 'Modifier' : 'Ajouter' }}</b-button>
  </b-form>
  </b-modal>
</template>
...
export default {
  ...
  methods: {
    onSubmit(ev) {
      ...
      this.$bvModal.hide(`modal-${this.edit? 'edit-' + this.product.id : 'add'}`);
    }
  }
}
</script>
